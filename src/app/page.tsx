import { ArrowRight, Baby, BatteryCharging, BriefcaseMedical, Globe2, Luggage, Plane, ShieldCheck, Sparkles } from 'lucide-react';
...
export default function Home() {
    ...
    <section className="relative overflow-hidden bg-gradient-to-br from-brand-50 via-white to-sky-50">
      <div className="mx-auto max-w-7xl px-5 py-6 md:px-8">
        <nav className="flex items-center justify-between">
          <a href="/" className="flex items-center gap-3 font-bold text-slate-950"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-brand-600 text-white"><Luggage className="h-5 w-5" /></span>Can I Bring It Now</a>
  ...
  We simplify travel rules, but always confirm important restrictions with official airline, airport or customs sources before travel.
    ...
    <section id="popular" className="mx-auto max-w-7xl px-5 py-14 md:px-8">
      <div className="flex items-end justify-between gap-4"><div><p className="font-semibold text-brand-600">Popular checks</p><h2 className="mt-2 text-3xl font-bold text-slate-950">Start with the highest-search travel questions</h2></div></div>
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        {rules.map(r => <a key={r.slug} href={`/rules/${r.slug}/`} className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-1 hover:shadow-soft">
          <p className="text-sm font-semibold text-brand-600">{r.category}</p><h3 className="mt-2 text-xl font-bold text-slate-950">{r.item}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{r.shortAnswer}</p><div className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-brand-600">Read rule <ArrowRight className="h-4 w-4" /></div>
        </a>)}
      </div>
    </section>
    ...
    <section id="categories" className="bg-slate-950 py-16 text-white">
      <div className="mx-auto max-w-7xl px-5 md:px-8">
        <div className="grid gap-8 md:grid-cols-3">
          <div className="rounded-3xl bg-white/10 p-6"><BatteryCharging className="h-8 w-8 text-sky-300" /><h3 className="mt-4 text-2xl font-bold">Batteries & electronics</h3><p className="mt-3 text-slate-300">Power banks, drones, cameras, laptops and lithium battery rules.</p></div>
          <div className="rounded-3xl bg-white/10 p-6"><BriefcaseMedical className="h-8 w-8 text-emerald-300" /><h3 className="mt-4 text-2xl font-bold">Medication & health</h3><p className="mt-3 text-slate-300">Medicine, prescriptions, liquid medication and destination restrictions.</p></div>
          <div className="rounded-3xl bg-white/10 p-6"><Baby className="h-8 w-8 text-pink-300" /><h3 className="mt-4 text-2xl font-bold">Baby travel</h3><p className="mt-3 text-slate-300">Baby milk, formula, food, prams and family travel essentials.</p></div>
        </div>
      </div>
    </section>
