  ...
  const submitSearch = (override?: string) => {
    ...
    if (q) {
      ...
      trackSearchEvent({
        ...
        bestMatchSlug: searchResults[0]?.slug,
        source: 'homepage',
      });
        ...
        {query.trim().length > 0 && results.length === 0 && (
          <div className="rounded-3xl border border-amber-100 bg-amber-50 p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-amber-800">
              Try a simpler phrase such as power bank, medication, baby formula, liquids or food.
              We use no-result searches to decide what to add next.
            </p>
          </div>
        )}
