...
import { getAffiliateSearchUrl, getRevenueProducts } from '@/lib/revenueEngine';
    ...
    <section className="mt-8 rounded-3xl bg-gradient-to-br from-white to-brand-50 p-6 ring-1 ring-slate-200">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-2 leading-7 text-slate-600">
            Product suggestions appear after the travel answer so users get help first, then optional useful products.
          </p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        {products.map((product) => (
          <a
            key={product.title}
            href={getAffiliateSearchUrl(product.searchTerm)}
            target="_blank"
          ...
          >
            ...
            <p className="mt-3 text-xs font-bold uppercase tracking-wide text-brand-600">{product.category}</p>
          </a>
        ))}
      </div>

      <p className="mt-4 text-xs leading-5 text-slate-500">
        Some links may become affiliate links. Recommendations should stay relevant and useful to travellers.
      </p>
    </section>
  );
}
