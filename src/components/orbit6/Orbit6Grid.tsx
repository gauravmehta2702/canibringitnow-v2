...
type Card = { title: string; href: string; label: string; description: string; channel?: string; effort?: string; };

export default function Orbit6Grid({ title, eyebrow, cards }: { title: string; eyebrow: string; cards: Card[] }) {
  return (
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        {cards.map((card) => (
          <a key={`${card.href}-${card.title}`} href={card.href} className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200 hover:bg-brand-50">
            <p className="text-xs font-black uppercase tracking-wide text-brand-600">{[card.effort, card.channel || card.label].filter(Boolean).join(' · ')}</p>
