...
import {
  sourceRegistry,
  requiredSourceTypes,
} from '@/data/travel-intelligence/sourceRegistry';
import type {
  GraphRecommendation,
...
function mentions(rule: Rule, name: string, slug: string) {
  ...
  return values.some((value) => {
    ...
    return (
      text.includes(` ${normalisedName} `) ||
      text.includes(` ${normalisedSlug} `)
    );
  });
}
...
function entityNode(
  input: Omit<TravelEntityNode, 'id'>,
): TravelEntityNode {
  return {
    ...input,
...
function buildRuleNodes(): TravelRuleNode[] {
  return rules.map((rule) => {
    const matchedAirlines = airlines
      .filter((airline) =>
        mentions(rule, airline, airlineSlug(airline)),
      )
      .map(airlineSlug);

    const matchedCountries = countries
      .filter((country) =>
        mentions(rule, country, countrySlug(country)),
      )
      .map(countrySlug);
...
function buildEdges(
  ...
  for (
    ...
    for (
      ...
      let type: TravelGraphEdgeType =
        'related-by-topic';

      if (left.itemSlug === right.itemSlug) {
        score += 34;
        reasons.push(
          'Same item in a different travel context',
        );
      ...
      if (
        ...
        score += 6;
        reasons.push(
          'Same cabin and checked-baggage outcome',
        );

        if (type === 'related-by-topic') {
          type = 'same-baggage-outcome';
        }
      }
...
export function buildTravelGraph(): TravelGraph {
  ...
  const byAirline = (slug: string) =>
    ruleNodes
      .filter((rule) =>
        rule.airlineSlugs.includes(slug),
      )
      .map((rule) => rule.id);

  const byCountry = (slug: string) =>
    ruleNodes
      .filter((rule) =>
        rule.countrySlugs.includes(slug),
      )
      .map((rule) => rule.id);

  const byCategory = (slug: string) =>
    ruleNodes
      .filter((rule) => rule.categorySlug === slug)
      .map((rule) => rule.id);

  const byItem = (slug: string) =>
    ruleNodes
      .filter((rule) => rule.itemSlug === slug)
      .map((rule) => rule.id);
...
export function getRuleGraphContext(
  slug: string,
): RuleGraphContext | undefined {
  ...
  const existingSourceTypes =
    new Set<SourceAuthorityType>(
      sources.map((source) => source.authorityType),
    );
  ...
  const required = requiredSourceTypes({
    ...
    category:
      graph.categories.find(
        (category) =>
          category.slug === rule.categorySlug,
      )?.name || rule.categorySlug,
  });

  return {
    rule,

    item: graph.items.find(
      (item) => item.slug === rule.itemSlug,
    ),

    category: graph.categories.find(
      (category) =>
        category.slug === rule.categorySlug,
    ),

    airlines: graph.airlines.filter((airline) =>
      rule.airlineSlugs.includes(airline.slug),
    ),

    countries: graph.countries.filter((country) =>
      rule.countrySlugs.includes(country.slug),
    ),

    guides: graph.guides.filter((guide) =>
      rule.guideSlugs.includes(guide.slug),
    ),

    sources,

    missingSourceTypes: required.filter(
      (type) => !existingSourceTypes.has(type),
    ),
  };
}

export function getGraphRecommendationsForRule(
  slug: string,
  limit = 8,
): GraphRecommendation[] {
  const graph = getTravelGraph();

  const current = graph.rules.find(
    (rule) => rule.slug === slug,
  );
  ...
  const ruleById = new Map(
    graph.rules.map((rule) => [rule.id, rule]),
  );
...
export function getGraphEntityLinksForRule(
  slug: string,
): GraphRecommendation[] {
  ...
  if (context.item) {
    links.push({
      ...
      description:
        'Open the complete item guide and compare related rules.',
      eyebrow: 'Item guide',
      score: 30,
      reasons: ['Primary item entity'],
    });
  }

  if (context.category) {
    links.push({
      href: context.category.href,
      title: context.category.name,
      description:
        'Browse the wider travel-rule category.',
      eyebrow: 'Category hub',
      score: 24,
      reasons: ['Primary category entity'],
    });
  }

  context.airlines.forEach((airline) => {
    links.push({
      href: airline.href,
      title: airline.name,
      description:
        'Review related baggage and restricted-item guidance for this airline.',
      eyebrow: 'Airline guide',
      score: 35,
      reasons: ['Airline named in this rule'],
    });
  });
  ...
  context.countries.forEach((country) => {
    links.push({
      ...
      title: country.name,
      description:
        'Review destination customs and arrival considerations.',
      eyebrow: 'Country guide',
      score: 35,
      reasons: ['Country named in this rule'],
    });
  });
...
export function getTravelGraphStats() {
  ...
  return {
    ...
    rulesWithAirlineLinks: graph.rules.filter(
      (rule) => rule.airlineSlugs.length > 0,
    ).length,

    rulesWithCountryLinks: graph.rules.filter(
      (rule) => rule.countrySlugs.length > 0,
    ).length,

    rulesWithNamedOfficialSources:
      graph.rules.filter((rule) =>
        rule.sourceIds.some(
          (id) =>
            id !==
            'editorial-general-travel-guidance',
        ),
      ).length,
  };
}
...
export function assessJourneyGraph(input: {
  ...
  const selectedAirlineSlug = input.airline
    ? airlineSlug(input.airline)
    : '';

  const selectedCountrySlug = input.destination
    ? countrySlug(input.destination)
    : '';

  const selected = input.ruleSlugs
    .map((slug) =>
      graph.rules.find((rule) => rule.slug === slug),
    )
  ...
  for (const rule of selected) {
    if (
      selectedAirlineSlug &&
      rule.airlineSlugs.length > 0 &&
      !rule.airlineSlugs.includes(
        selectedAirlineSlug,
      )
    ) {
      const named = graph.airlines
        .filter((airline) =>
          rule.airlineSlugs.includes(airline.slug),
        )
      ...
      alerts.push(
        `${rule.title} is based on ${
          named || 'another airline'
        } guidance, but your selected airline is ${
          input.airline
        }. Verify ${
          input.airline
        }'s current policy separately before travelling.`,
      );
    ...
    if (
      selectedCountrySlug &&
      rule.countrySlugs.length > 0 &&
      !rule.countrySlugs.includes(
        selectedCountrySlug,
      )
    ) {
      const named = graph.countries
        .filter((country) =>
          rule.countrySlugs.includes(country.slug),
        )
      ...
      alerts.push(
        `${rule.title} contains destination guidance for ${
          named || 'another country'
        }, but your selected destination is ${
          input.destination
        }. Check ${
          input.destination
        } customs and import rules separately.`,
      );

      score -= 8;
    }

    const context = getRuleGraphContext(
      rule.slug,
    );
  ...
  if (officialSourceGapCount > 0) {
    alerts.push(
      `${officialSourceGapCount} selected rule${
        officialSourceGapCount === 1 ? '' : 's'
      } still need named official-source verification.`,
    );

    score -= Math.min(
      15,
      officialSourceGapCount * 3,
    );
  }

  const recommendationMap =
    new Map<string, GraphRecommendation>();
  ...
  selected.forEach((rule) => {
    getGraphRecommendationsForRule(
      ...
      const isAlreadySelected =
        input.ruleSlugs.some((slug) =>
          recommendation.href.includes(
            `/rules/${slug}/`,
          ),
        );
  ...
  const label: JourneyGraphAssessment['label'] =
    bounded >= 85
      ? 'Well connected'
      : bounded >= 65
        ? 'Strong context'
